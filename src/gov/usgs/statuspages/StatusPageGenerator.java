...
public class StatusPageGenerator {
    ...
    public void startThreads(String directory) {
        ...
        // Start tasks
        for (SaveChartTask aTask: tasks) {
            threadPool.scheduleAtFixedRate(aTask, initDelay, aTask.getInterval(), TimeUnit.SECONDS);
        }
    }
	...
	private void writePHP(String directory) {
		...
		for (SaveChartTask task: this.tasks) {
		    output.println("\'" + task.getFilename() + "\'\t=>\t\'/valve3/valve3.jsp?" + task.getURL() + "\',");
		}
     ...
     * arguments and uses those values to create the StatusPageGenerator object which handles all
     * the multithreading
     * 
     * @param args
     */
